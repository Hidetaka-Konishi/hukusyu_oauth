...
engine = create_engine('sqlite:///hukusyu_when.db')
# データベースセッションを作成する準備
Session = sessionmaker(bind=engine)
# データベースセッションをインスタンス化してデータベースとの接続を開始
session = Session()
...
class Database(Base):
    ...
    def default(self, user_email, list_or_dict):
        list_or_dict_dumps = json.dumps(list_or_dict)            
        list_or_dict_entry = Database(email=user_email, data=list_or_dict_dumps)
        session.add(list_or_dict_entry)
        session.commit()


    # データベーステーブルからデータを取り出す。offset_numberには取得したいデータベーステーブルの行数から1引いた数字を指定する。
    def query(self, user_email, offset_number):
        # データベーステーブルから取得した復元されていないデータ
        global query_offset
        # 取得したいデータベーステーブルの行を一行だけ取得
        query_offset = session.query(Database).filter_by(email=user_email).offset(offset_number).first()
